...
async function run() {
  try {
    ...
    console.log('Updating dates to July 2, 2026...');

    let updatedCount = 0;
    for (const c of exitedCases) {
      const originalExitDate = c.stageTimestamps.exited;
      
      // Update exited timestamp to July 2, 2026
      const newExitDate = new Date(originalExitDate);
      newExitDate.setUTCDate(2); // Set day of month to 2
